...
<p> Nous tirons aléatoirement un déchet par la fonction <code>flux(p)</code> et nous observons si le déchet ira dans la bonne ou dans la mauvaise poubelle grâce à la fonction `poubelle-choisie(p,q) suivante, avec q, le dictionnaire de probabilités de répartition des déchets: </p>
